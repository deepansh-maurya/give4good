...
export const FundRaisingCard = ({ campaign, mycamp }) => {
  ...
  async function handleProfile() {
    if (beneficieryid != creatorid) {
      ...
      let res = await getProfile(creatorid, true, false);
      setcreator(res);
      setbeneficiery(res);
    }
  }
      ...
      <div className="bg-white w-[430px] dark:bg-black  text-white shadow-black  mt-3 shadow-lg rounded-lg overflow-hidden">
        ...
        <div className="p-4">
          ...
          <div className="bg-gray-200 dark:bg-gray-700 h-3 rounded-lg mb-4 overflow-hidden">
            <div
              className="bg-red-500   border-2 border-red-500  h-full"
              style={{
                width: `${
                  (Number(campaign.progress) || 0 / Number(campaign.goal)) * 100
                }%`,
              }}
            ></div>
          </div>
          <div className="flex justify-between">
            <button
              onClick={() => {
                if (mycamp == true) {
                  console.log("dfgd");
                  nav("/my-campaign-page", {
                    state: { campaign },
                  });
                } else nav("/campaign-page", { state: { campaign } });
              }}
